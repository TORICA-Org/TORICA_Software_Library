<<<<<<< HEAD:TORICA_WebServer_lib/index.h
//WebページのHTMLです
  ...
  <script type='text/javascript'>
    function updateData() {
      const getData = fetch('/get/data')
        ...
        .then((data) => {
          console.log(data);
          const index = data.substr(0,2); // 0番目から2つ
          const content = data.substr(2); // 2番目から最後まで

          if (document.getElementById(index) === null) {
            const span = document.createElement('span');
            span.setAttribute('id', index);
            span.setAttribute('name', 'content');
            document.getElementById('monitor').append('span');
          }
...
#endif // INDEX_H
>>>>>>> 52fab5a81cc0a9454ff2176d702e0a7181f3abec:TORICA_WebServer_lib/src/index.h
